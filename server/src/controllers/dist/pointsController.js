...
var PointsController = /** @class */ (function () {
    ...
    PointsController.prototype.index = function (request, responde) {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, connection_1["default"]('points').join('point_items', 'points.id', '=', 'point_items.point_id')
                                .whereIn('point_items.item_id', parsedItems)
                                .where('city', String(city)).where('uf', String(uf)).distinct().select('points.*')];
                    case 1:
                        points = _b.sent();
                        return [2 /*return*/, express_1.response.json(points)];
                }
            });
        });
    };
    PointsController.prototype.show = function (request, responde) {
        return __awaiter(this, void 0, void 0, function () {
            var id, point, items;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        point = _a.sent();
                        if (!point) {
                            return [2 /*return*/, express_1.response.status(400).json({ message: 'Point not found' })];
                        }
                        return [4 /*yield*/, connection_1["default"]('items').join('point_items', 'items.id', '=', 'point_items.items_id')
                                .where('point_items.point_id', id).select('items.title')];
                    case 2:
                        items = _a.sent();
                        return [2 /*return*/, express_1.response.json({ point: point, items: items })];
                }
            });
        });
    };
